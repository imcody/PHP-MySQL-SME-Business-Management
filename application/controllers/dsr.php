...
class dsr extends CI_Controller {
    ...
    public function lstclnt(){
        ...
        $this->Merged_Vars['clnts']     = $this->clientsmodel->get_client_list_for_sales(2, $this->Sesson_Vars['role']);
    ...
    public function newclnt(){
        if($this->input->post('trigger') && ($this->input->post('trigger') == "createclient")){
            $this->form_validation->set_rules('cl_national_id', 'Customer National ID', 'required|xss_clean|is_unique[client.cl_national_id]');
            if($this->form_validation->run() == FALSE){
                $this->session->set_flashdata('notification', errormessage(validation_errors()));
                redirect(__CLASS__ . '/new', 'refresh');
            } else {
                $Visible = 0;
                if($this->input->post('visible') == 'on'){ $Visible = 1; }
                $customer_array = array('cl_name' => $this->input->post('cl_name'), 'cl_phone_no' => $this->input->post('cl_phone_no'), 'cl_mobile_no' => $this->input->post('cl_mobile_no'), 'cl_national_id' => $this->input->post('cl_national_id'), 'cl_email' => $this->input->post('cl_email'), 'cl_balance' => $this->input->post('cl_balance'), 'cl_type' => $this->input->post('cl_type'), 'visible' => $Visible);
    ...
    public function editclnt(){
        if($this->input->post('trigger') && ($this->input->post('trigger') == "updateclient")){
            $Visible = 0;
            if($this->input->post('visible') == 'on'){ $Visible = 1; }
            $customer_array = array('cl_id' => $this->input->post('clid'), 'cl_name' => $this->input->post('cl_name'), 'cl_phone_no' => $this->input->post('cl_phone_no'), 'cl_mobile_no' => $this->input->post('cl_mobile_no'), 'cl_national_id' => $this->input->post('cl_national_id'), 'cl_email' => $this->input->post('cl_email'), 'cl_type' => $this->input->post('cl_type'), 'visible' => $Visible);
            ...
            if($score){
                ...
                $this->session->set_flashdata('notification', errormessage());
                redirect(__CLASS__ . '/update?cl_id=' . $this->input->post('clid'), 'refresh');
            }
        }
        else{
    ...
    public function dsr_ledger(){
        $this->Merged_Vars['printer']       = 'DSR Ledger List';
        $this->Merged_Vars['back_link']     = 'dsr/list';
