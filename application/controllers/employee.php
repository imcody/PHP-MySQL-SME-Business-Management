...
class employee extends CI_Controller {
    ...
    public function salary_closing(){
        $emp_id = $this->input->post('emp_id'); $ledger_id = 2; $ledger_type = 1; $_amount = $this->input->post('amount'); $_emp_bal = 0; $_balance = $this->input->post('balance');
//        Updated on 8th of July, 2018.
        ...
        $_ledger_       = array('emp_id'    => $emp_id, 'ledger_id'    => $ledger_id, 'ledger_type'    => $ledger_type, 'amount'    => $_amount, 'balance'    => $_emp_bal, 'notes'    => $notes, 'ledger_date'    => $this->input->post('sc_ledger_date'), 'posting_date'    => date('Y-m-d H:i:s'), 'posting_by'    => $this->Merged_Vars['memb']);
        $salary_closing = array('EmployeeID' => $emp_id, 'Month' => $month, 'Year' => $year, 'Amount' => $_amount, 'PostedOn' => date('Y-m-d'), 'PostedBy' => $this->Merged_Vars['memb']);
        $emp_data       = array('tble_id' => $emp_id, 'emp_opening_balance' => $_emp_bal);
        $closed_sal     = $this->employeemodel->get_employee_closed_salary($emp_id, $month, $year);
        $monthly_salary = $this->input->post('monthly_salary');
        if($closed_sal > $monthly_salary){
            $this->session->set_flashdata('notification', errormessage('Closing is already done for this month.'));
        ...
        $score          = $this->employeemodel->set_employee_ledger($_ledger_, $emp_data, $salary_closing);
        if($score){
    ...
    public function new_employee(){
        if($this->input->post('trigger') && ($this->input->post('trigger') == "createemployee")){
            $this->form_validation->set_rules('emp_monthly_salary', 'Monthly Salary', 'required|integer|less_than[50000]|greater_than[1500]');
    ...
    public function get_employee_salary_closing_log(){
        $month                                  = $this->input->post('month');
        $years                                  = $this->input->post('year');
        $emp_id                                 = $this->input->post('cl_id');
        $this->Merged_Vars['closing']           = $this->employeemodel->get_employee_salary_closing_log($month, $years, $emp_id);
        $this->load->view(__CLASS__ . '/' .  __FUNCTION__, $this->Merged_Vars);
    }
    ...
    public function ledger(){
        ...
        $this->Merged_Vars['employeed_id']  = $this->input->get('cl_id');
        $this->loadView(__CLASS__, __FUNCTION__, $this->Merged_Vars);
    }
}
?>