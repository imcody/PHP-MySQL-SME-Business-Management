...
//                    $this->db->update('employee', $emp_data);
    ...
    public function get_employee_salary_closing_log($month, $years, $empid){
        $this->db->select('*');
        $this->db->from('employee_salary_closing');
        $this->db->where('employee_salary_closing.Month', $month);
        $this->db->where('employee_salary_closing.Year', $years);
        $this->db->where('employee_salary_closing.EmployeeID', $empid);
        $Closing = $this->db->get();
    ...
    public function get_employee_closed_salary($emp, $month, $year){
        $closed = 0;
        $this->db->select('employee_salary_closing.Amount');
        $this->db->from('employee_salary_closing');
        $this->db->where('employee_salary_closing.EmployeeID', $emp);
        $this->db->where('employee_salary_closing.Month', $month);
        $this->db->where('employee_salary_closing.Year', $year);
        $Amount = $this->db->get();
        if($Amount->num_rows() > 0){
            foreach($Amount->result() as $Salary){
                $closed = $closed + $Salary->Amount;
            }
            return $closed;
        } else {
            return $closed;
    ...
    public function set_employee_ledger($_ledger_, $emp_data, $salary_closing){
        $this->db->trans_begin();
        $this->db->insert('employee_salary_closing', $salary_closing);
